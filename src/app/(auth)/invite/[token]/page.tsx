...
const ROLE_LABEL: Record<string, string> = {
  admin: '관리자', manager: '팀장', sales: '영업담당자'
}
...
export default function AcceptInvitePage() {
  ...
  useEffect(() => {
    const verify = async () => {
      ...
      setInvite(mockPayload)
      setName(mockPayload.name)
      setTokenState('valid')
    }
    verify()
  }, [token])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrMsg('계정 설정 중 오류가 발생했습니다. 다시 시도해 주세요.')
    }
  }
      ...
      <div className="min-h-screen bg-gray-950 flex items-center justify-center p-4">
        <div className="max-w-sm w-full text-center">
          ...
          <p className="text-sm text-gray-400 mb-6">
            {tokenState === 'expired'
              ? '초대 링크는 7일간 유효합니다. 관리자에게 재초대를 요청하세요.'
              : '이미 계정이 설정되었습니다. 로그인 페이지로 이동하세요.'}
          </p>
          <button
      ...
      <div className="min-h-screen bg-gray-950 flex items-center justify-center p-4">
        <div className="max-w-sm w-full text-center">
          ...
          <p className="text-sm text-gray-400 mb-1">
            <span className="text-white font-medium">{invite?.tenant_name}</span>에 합류했습니다
          </p>
          <p className="text-sm text-gray-500 mb-6">
            {invite?.email} · {ROLE_LABEL[invite?.role ?? 'sales']}
          </p>
          <button
            onClick={() => router.push('/login')}
            className="w-full py-2.5 bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold rounded-xl transition-all shadow-lg shadow-blue-600/20"
          >
            로그인하기
          </button>
        </div>
      ...
      <div className="relative w-full max-w-sm">
        ...
        <div className="bg-gray-800/60 border border-gray-700/50 rounded-2xl p-6 backdrop-blur-sm shadow-2xl">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className={cn(
                  ...
                  'bg-gray-700/60 border focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors',
              ...
              {passwordCf && password === passwordCf && (
                <p className="text-[10px] text-green-400 mt-1">비밀번호가 일치합니다</p>
              )}
            </div>
            ...
            >
              {submitState === 'loading' ? (
                <><Loader2 className="w-4 h-4 animate-spin" />계정 설정 중...</>
              ) : (
                '계정 설정 완료'
              )}
            </button>
