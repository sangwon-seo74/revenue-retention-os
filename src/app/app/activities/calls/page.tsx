...
function QuickCallModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative bg-dk-surface border border-dk-border rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-lg mx-0 sm:mx-4 p-6">
        <div className="flex items-center justify-between mb-5">
          ...
          <button onClick={onClose} className="text-dk-muted hover:text-dk-text text-xl leading-none">×</button>
        </div>
        ...
        <div className="mb-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="고객사 검색..."
              className={'pl-9 pr-3 py-2 ' + INPUT_CLS}
            />
          </div>
        </div>
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="flex-1 px-4 py-2.5 text-sm font-medium text-white bg-[#1f6feb] rounded-lg hover:bg-[#388bfd] disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
          >
            저장
          </button>
        </div>
      ...
      <div className="p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 text-xs text-dk-dim">
...
export default function CallsPage() {
  ...
  useEffect(() => {
    fetch('/api/activities?type=call&limit=50')
      .then(r => r.json())
      .then(json => { setActivities((json.data?.data ?? []) as Activity[]); setLoading(false) })
