...
import {
  ...
  CheckCircle2, ArrowRight, TrendingUp, X
} from 'lucide-react'
...
const MOCK_PLANS: Plan[] = [
  {
    ...
    monthly_price: 0, yearly_price: 0, is_active: true, created_at: '2024-01-01T00:00:00Z'
  },
  {
    id: 'plan_standard', name: 'Standard', code: 'standard',
    max_users: 10, max_companies: 300, max_messages: 5000,
    monthly_price: 99000, yearly_price: 990000, is_active: true, created_at: '2024-01-01T00:00:00Z'
  },
  {
    id: 'plan_pro', name: 'Pro', code: 'pro',
    max_users: null, max_companies: null, max_messages: null,
    monthly_price: 249000, yearly_price: 2490000, is_active: true, created_at: '2024-01-01T00:00:00Z'
  },
]
...
function UpgradeModal({ current, plans, onClose }: {
  current: Plan; plans: Plan[]; onClose: () => void
}) {
    ...
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-dk-surface border border-dk-border rounded-2xl shadow-xl w-full max-w-2xl">
        <div className="flex items-center justify-between px-6 py-5 border-b border-dk-border">
          <h2 className="text-base font-bold text-dk-text">플랜 변경</h2>
          ...
          <div className="inline-flex bg-dk-surface2 rounded-xl p-1 gap-1">
            {(['monthly', 'yearly'] as BillingCycle[]).map(c => (
              <button key={c} onClick={() => setCycle(c)}
                ...
                {c === 'monthly' ? '월간' : '연간'}
                {c === 'yearly' && (
                  <span className="ml-1.5 text-xs text-dk-green font-semibold">2개월 할인</span>
                )}
              </button>
            ))}
          </div>
            ...
            return (
              <div key={plan.id} onClick={() => setSelected(plan.id)}
                className={cn(
                  ...
                  selected === plan.id
                    ? 'border-dk-blue bg-dk-blue/5'
                    : 'border-dk-border hover:border-dk-border2'
                )}>
                {plan.code === 'pro' && (
                ...
                {isCurrent && (
                  <div className="absolute -top-2.5 right-3 text-xs bg-dk-surface2 text-dk-muted px-2 py-0.5 rounded-full border border-dk-border">
                    현재
                  </div>
                )}
                <p className="font-bold text-dk-text">{plan.name}</p>
                <p className="text-2xl font-bold text-dk-text mt-2">
                  {price === 0 ? '무료' : formatAmount(price)}
                </p>
                {price > 0 && (
                  <p className="text-xs text-dk-dim">{cycle === 'monthly' ? '/월' : '/년'}</p>
                )}
                <ul className="mt-4 space-y-1.5">
                  {PLAN_FEATURES[plan.code].map(f => (
                    <li key={f} className="flex items-center gap-1.5 text-xs text-dk-muted">
                      <CheckCircle2 className="w-3.5 h-3.5 text-dk-green shrink-0" />
                      {f}
                    </li>
                  ))}
                </ul>
        ...
        <div className="px-6 pb-6 flex gap-3">
          <button onClick={onClose}
            className="flex-1 py-2.5 text-sm text-dk-muted border border-dk-border rounded-xl hover:bg-dk-surface2">
            취소
          </button>
          <button disabled={selected === current.id}
            className="flex-1 py-2.5 text-sm text-white bg-dk-blue rounded-xl hover:bg-dk-blue/80 disabled:opacity-50 font-medium">
            플랜 변경 신청
          </button>
        </div>
      </div>
...
function UsageBar({ label, current, max, icon: Icon }: {
  ...
  const color = pct >= 90 ? 'bg-dk-red' : pct >= 70 ? 'bg-dk-orange' : 'bg-dk-blue'

  return (
    <div>
      <div className="flex items-center justify-between mb-1.5">
        <div className="flex items-center gap-1.5 text-sm text-dk-text">
          <Icon className="w-3.5 h-3.5 text-dk-muted" />
          {label}
        </div>
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="bg-dk-surface border border-dk-border rounded-xl p-5">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-dk-muted mt-0.5">더 많은 기능이 필요하시면 Pro 플랜으로 업그레이드하세요</p>
          </div>
      ...
      <div className="bg-dk-surface border border-dk-border rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <button className="text-xs text-dk-blue hover:text-dk-blue/80">변경</button>
        </div>
        <div className="flex items-center gap-3 p-3 bg-dk-surface2 rounded-xl border border-dk-border">
          ...
          <div>
            <p className="text-sm font-medium text-dk-text">•••• •••• •••• 1234</p>
            <p className="text-xs text-dk-dim">만료 12/26</p>
          </div>
          <CheckCircle2 className="w-4 h-4 text-dk-green ml-auto" />
        </div>
      </div>
      ...
      <div className="bg-dk-surface border border-dk-border rounded-xl p-5">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-sm font-semibold text-dk-text">구독 해지</h3>
            <p className="text-xs text-dk-muted mt-0.5">해지 시 현재 결제 기간 종료 후 서비스가 종료됩니다</p>
          </div>
          <button className="text-xs text-dk-red border border-dk-red/30 px-3 py-1.5 rounded-lg hover:bg-dk-red/10">
            해지 신청
          </button>
        </div>
      </div>
      ...
      {showUpgrade && (
        <UpgradeModal current={sub.plan} plans={MOCK_PLANS} onClose={() => setShowUpgrade(false)} />
      )}
    </div>
  )
}
