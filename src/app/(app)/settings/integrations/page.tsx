...
import { Plug, CheckCircle, XCircle, RefreshCw, Eye, EyeOff, AlertCircle } from 'lucide-react'
...
const INTEGRATIONS: Integration[] = [
  ...
  },
  {
    ...
    is_active: true,
    tested_at: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString(),
    fields: [
...
function IntegrationCard({ integration }: { integration: Integration }) {
  ...
  return (
    <div className={`bg-white dark:bg-slate-900 border rounded-xl overflow-hidden transition-all
      ${integration.is_active ? 'border-slate-200 dark:border-slate-800' : 'border-slate-200 dark:border-slate-800 opacity-75'}`}>
      {/* 헤더 */}
      <div className="flex items-center gap-3 p-4">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <span className="text-sm font-semibold text-slate-900 dark:text-slate-100">{integration.label}</span>
        ...
        <div className="flex items-center gap-2">
          {integration.tested_at && (
            <span className="text-xs text-slate-400">테스트: {formatTested(integration.tested_at)}</span>
          )}
          <button
            onClick={() => setExpanded(!expanded)}
            className="px-3 py-1.5 text-xs font-medium text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800"
          >
            {expanded ? '접기' : '설정'}
          </button>
        </div>
      ...
      {expanded && (
        <div className="border-t border-slate-100 dark:border-slate-800 p-4 bg-slate-50/50 dark:bg-slate-900/50">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-medium text-slate-500">API 설정</span>
            <button
            ...
            {integration.fields.map(field => (
              <div key={field.key}>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 text-sm border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono"
                />
              </div>
            ))}
          </div>
          ...
          <div className="flex items-center gap-2 mt-4">
            <button
              onClick={handleTest}
              disabled={testing}
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-slate-600 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-white dark:hover:bg-slate-800 disabled:opacity-50"
            >
            ...
            {integration.is_active && (
              <button className="ml-auto px-3 py-1.5 text-xs font-medium text-red-600 border border-red-200 rounded-lg hover:bg-red-50">
                연동 해제
              </button>
            )}
          </div>
...
export default function IntegrationsSettingPage() {
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">API 연동</h2>
        <p className="text-sm text-slate-500 mt-0.5">외부 서비스와 연동하여 메시지 발송 기능을 활성화합니다</p>
      </div>

      <div className="p-3 bg-amber-50 dark:bg-amber-950/50 border border-amber-200 dark:border-amber-800 rounded-xl flex items-start gap-2">
