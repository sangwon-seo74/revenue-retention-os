...
function TabBasic({ tenant, onUpdate }: { tenant: TenantDetail; onUpdate: () => void }) {
  ...
  const resendInvite = async () => {
    ...
    setProcessing(true)
    setActionMsg(null)
    const res  = await fetch(`/api/super-admin/tenants/${tenant.id}/resend-invite`, { method: 'POST' })
    const json = await res.json()
    setProcessing(false)
    if (!res.ok || json.error) setActionMsg({ type: 'error', text: json.error?.message ?? '재발송 실패' })
    else                       setActionMsg({ type: 'success', text: json.data?.message ?? '재발송 완료' })
  }
  ...
  const resetPassword = async () => {
    ...
    setProcessing(true)
    setActionMsg(null)
    const res  = await fetch(`/api/super-admin/tenants/${tenant.id}/reset-password`, { method: 'POST' })
    const json = await res.json()
    setProcessing(false)
    if (!res.ok || json.error) {
      setActionMsg({ type: 'error', text: json.error?.message ?? '발급 실패' })
      return
    }
    setTempPw({ email: json.data.user.email, password: json.data.temp_password })
  }
...
function TabUsers({ tenantId, users, onUpdate }: {
  ...
  const impersonate = async (uid: string) => {
    ...
    const json = await res.json()
    setActingId(null)
    if (res.ok && json.data?.impersonation_link) setImpLink(json.data.impersonation_link as string)
    else alert(json.error?.message ?? '가장 링크 생성 실패')
  }
